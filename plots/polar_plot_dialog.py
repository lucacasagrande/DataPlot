...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), '../ui/Polar.ui'))

class PolarPlotDialog(QtGui.QDialog, FORM_CLASS):
    def __init__(self, parent=None):
        """Constructor."""
        super(PolarPlotDialog, self).__init__(parent)
        ...
        self.setupUi(self)
        self.buttonBox.button(QDialogButtonBox.Ok).clicked.connect(self.Polar)
    ...
    def addNewTrace(self):
        ...
        self.traceTable.setItem(row, 5, QTableWidgetItem(str(self.colorButton.color().name())))
        self.traceTable.setItem(row, 6, QTableWidgetItem(str(self.alpha.value())))
    ...
    def Polar(self):
        ...
        for key in self.superdict:
            r = self.superdict[key].get('R')
            t = self.superdict[key].get('T')
